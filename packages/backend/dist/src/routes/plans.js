...
async function planRoutes(app) {
    ...
    `).run(walletId, body.data.destination, body.data.fee_rate, result.estimatedFee, result.outputAmount, JSON.stringify(result.warnings), Date.now()).lastInsertRowid;
        const insertInput = db.prepare('INSERT INTO plan_inputs (plan_id, txid, vout, amount) VALUES (?, ?, ?, ?)');
    ...
    app.get('/api/plans/:planId', async (req, reply) => {
        ...
        const inputs = db.prepare('SELECT * FROM plan_inputs WHERE plan_id = ?').all(plan.id);
        // Build label map for inputs
        const inputLabels = {};
        for (const inp of inputs) {
            const ref = `${inp.txid}:${inp.vout}`;
            const lbl = db.prepare("SELECT name FROM labels WHERE wallet_id = ? AND ref = ? AND label_type = 'output'").get(plan.wallet_id, ref);
            if (lbl)
                inputLabels[ref] = lbl.name;
        }
