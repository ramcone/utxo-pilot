...
exports.feeHistoryRoutes = feeHistoryRoutes;
...
const CACHE_TTL = {
    ...
    '1w': 15 * 60000, // 15 min
    '1m': 60 * 60000, // 1 hour
    '1y': 6 * 60 * 60000, // 6 hours
};
// mempool.space interval param per range
const MEMPOOL_INTERVAL = {
    '1d': '24h',
    '1w': '1w',
    '1m': '1m',
    '1y': '1y',
};
async function feeHistoryRoutes(app) {
    app.get('/api/fees/history', async (req, reply) => {
        ...
        let data;
        if (range === '1h') {
            data = getLocalHistory((0, database_js_1.getDb)());
        }
        else {
            try {
...
function getLocalHistory(db) {
    const since = Date.now() - 60 * 60 * 1000;
    const rows = db.prepare('SELECT fetched_at, fastest, half_hour, hour FROM fee_snapshots WHERE fetched_at >= ? ORDER BY fetched_at ASC').all(since);
...
async function getMempoolHistory(range) {
    const interval = MEMPOOL_INTERVAL[range];
    const res = await (0, node_fetch_1.default)(`https://mempool.space/api/v1/mining/blocks/fee-rates/${interval}`, { headers: { 'User-Agent': 'utxo-pilot/0.1.0' } });
    if (!res.ok)
